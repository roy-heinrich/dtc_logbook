        ...
        <form method="GET" action="{{ route('admin.reports.index') }}" class="space-y-6">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <form id="exportPdfForm" method="POST" action="{{ route('admin.export.pdf') }}" style="display: inline;">
                        @csrf
                        <input type="hidden" name="start_date" id="pdfStartDate">
                        <input type="hidden" name="end_date" id="pdfEndDate">
                        <input type="hidden" name="service_type" id="pdfServiceType">
                        <button type="submit" class="rounded-xl bg-rose-500 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-600">
                            Export PDF
                        </button>
                    </form>
                </div>
...
<script>
    ...
    async function updatePreview() {
        ...
        try {
            ...
            document.getElementById('pdfStartDate').value = filters.startDate;
            document.getElementById('pdfEndDate').value = filters.endDate;
            document.getElementById('pdfServiceType').value = filters.serviceType;
        } catch (error) {
